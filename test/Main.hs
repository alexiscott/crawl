...
foo :: Int -> (Int, Int)
foo 3 = (1, 2)
foo _ = (0, 0)  -- Default case for other inputs

partA :: Int -> IO (Int, Int)
partA 3 = return (5, 10)  -- Example tuple, 10 can be any value
partA _ = return (0, 0)   -- Default case for other inputs

partB :: Int -> IO Bool
partB 10 = return True  -- Expected to return True for y = 10
partB _  = return False -- Returns False for any other value

tests = test [ "test1" ~: "(foo 3)" ~: (1,2) ~=? (foo 3),
               "test2" ~: "narrowTags contains Tags with class of 'athing'" ~: (TagOpen "tr" [("class","athing"),("id","41399872")]) ~=? head (head $ narrowTags hackerNewsSample),
               "test3" ~: "extracts a Point from an Article" ~:  (TagText "1.") ~=? getRank,
               "test4" ~: "extracts a Point from an Article" ~:  Just "1." ~=? maybeTagText getRank,
               "test5" ~: "extracts a Titleline from an Article" ~:  Just "1." ~=? maybeTagText getRank
...
getRank :: Tag String
getRank = (head $ narrowTags hackerNewsSample) !! 4

-- Current example Hacker News content.
hackerNewsSample :: String
hackerNewsSample = "<!DOCTYPE html>\
